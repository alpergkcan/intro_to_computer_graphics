...
{
	// variables
	vec3 objColor = texture(MoonTexColor, TexCoord).xyz; // vec3(0.3, 0.3, 0.3)
	...
	vec3 halfVector = (lightRay + cameraRay ) / 2.0f;

	vec3 ambient  = ambientRC * ambientLight;
	vec3 diffuse  =     max(dot(lightRay,   Normal), 0) * objColor;
